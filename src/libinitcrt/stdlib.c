...
malloc_smallpool_extend(struct malloc_arena *arena) {
	...
	if (mmaped != MAP_FAILED) {
		...
		while (*iterator = (uint8_t *)iterator + (1 << MALLOC_MEDIUMPOOL_MIN_ORDER), *iterator < allocationend) {
			iterator = *iterator;
		}
...
malloc_mediumpool_extend(struct malloc_arena *arena) {
	...
	if (mmaped != MAP_FAILED) {
		...
		while (*iterator = (uint8_t *)iterator + (1 << (MALLOC_HUGEPOOL_MIN_ORDER - 1)), *iterator < allocationend) {
			iterator = *iterator;
		}
